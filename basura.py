...
# ========== VISUALIZACIÓN 3D DE DATOS ORIGINALES ==========
        fig = plt.figure(figsize=(8, 6))
        ax = fig.add_subplot(111, projection='3d')

        x = X_train[:, 0]  # Horas de estudio
        y = X_train[:, 1]  # Nota previa
        z = X_train[:, 2]  # Asistencia
        labels = y_train_clasificado  # Clases binarizadas

        scatter = ax.scatter(x, y, z, c=labels, cmap='coolwarm', s=60, edgecolors='k')
        ax.set_xlabel("Horas de estudio")
        ax.set_ylabel("Nota previa")
        ax.set_zlabel("Asistencia")
        ax.set_title("Visualización 3D de datos originales")
        plt.tight_layout()
        plt.show()


        # ========== VISUALIZACIÓN 3D DE VECTORES DE SOPORTE ==========
        # Entrenar un modelo final para visualizar soporte (por ejemplo: RBF con C=1, gamma=0.1)
        modelo_final_3d = SVC(kernel='rbf', C=1, gamma=0.1)
        modelo_final_3d.fit(X_train, y_train_clasificado)

        vectores_soporte = modelo_final_3d.support_
        ...
        ax.scatter(X_train[:, 0], X_train[:, 1], X_train[:, 2],
                c=y_train_clasificado, cmap='Set1', s=60, edgecolors='k', label='Datos')

        # Vectores de soporte resaltados
        ax.scatter(X_train[vectores_soporte, 0],
           X_train[vectores_soporte, 1],
           X_train[vectores_soporte, 2],
           c=y_train_clasificado[vectores_soporte],
           cmap='Set2', edgecolors='k', s=100, label='Vectores de soporte')


        ax.set_xlabel("Horas de estudio")
        ax.set_ylabel("Nota previa")
        ax.set_zlabel("Asistencia")
        ax.set_title("Vectores de soporte en espacio 3D original")
        ax.legend()
        plt.tight_layout()
        plt.show()
...
# ========== VISUALIZACIÓN 3D CON COLORES CLAROS Y CONTRASTANTES ==========

        # Entrenamos un modelo RBF para obtener vectores de soporte
        modelo_final_3d = SVC(kernel='rbf', C=1, gamma=0.1)
        modelo_final_3d.fit(X_train, y_train_clasificado)
        vectores_soporte = modelo_final_3d.support_
        ...
        # === Datos de entrenamiento: tonos de azul (cool)
        ax.scatter(X_train[:, 0], X_train[:, 1], X_train[:, 2],
                c=y_train_clasificado,
                cmap='Blues',                # Mapa azul
                s=60,
                edgecolors='k',
                label='Datos')

        # === Vectores de soporte: naranja brillante
        ax.scatter(X_train[vectores_soporte, 0],
                X_train[vectores_soporte, 1],
                X_train[vectores_soporte, 2],
                color='orange',             # Naranja sólido
                edgecolors='black',         # Bordes negros para destacarlos
                s=100,
        ...
        ax.set_ylabel("Nota previa")
        ax.set_zlabel("Asistencia")
        ax.set_title("SVM - Vectores de soporte destacados")
...
# # === GRÁFICO 3D DE ACCURACY EN FUNCIÓN DE C Y GAMMA ===

# gammas = [0.01, 0.1, 1]
# C_values = [0.1, 1, 10]

# accuracy_matrix = np.zeros((len(C_values), len(gammas)))
# C_list = []
# gamma_list = []
# acc_list = []
...
#         modelo = SVC(kernel='rbf', C=C, gamma=gamma)
#         modelo.fit(X_train, y_train_clasificado)
#         y_pred = modelo.predict(X_test)
#         acc = accuracy_score(y_test_clasificado, y_pred)
#         accuracy_matrix[i, j] = acc

#         # Guardar valores para scatter
#         C_list.append(C)
#         gamma_list.append(gamma)
#         acc_list.append(acc)

# # Crear grilla para la superficie
# C_grid, gamma_grid = np.meshgrid(C_values, gammas, indexing='ij')

# fig = plt.figure(figsize=(10, 7))
# ax = fig.add_subplot(111, projection='3d')

# # Superficie
# ax.plot_surface(C_grid, gamma_grid, accuracy_matrix, cmap='viridis', edgecolor='k', alpha=0.7)

# # Puntos (scatter) individuales
# ax.scatter(C_list, gamma_list, acc_list, color='red', s=50, label='Puntos evaluados')

# ax.set_title("Accuracy en función de C y gamma (SVM RBF)")
# ax.set_xlabel("C")
# ax.set_ylabel("Gamma")
...
# # # Crear grilla para el gráfico 3D
# # C_grid, gamma_grid = np.meshgrid(C_values, gammas, indexing='ij')

# # fig = plt.figure(figsize=(10, 7))
# # ax = fig.add_subplot(111, projection='3d')
# # ax.plot_surface(C_grid, gamma_grid, accuracy_matrix, cmap='viridis', edgecolor='k')
# # ax.set_title("Accuracy en función de C y gamma (SVM RBF)")
# # ax.set_xlabel("C")
# # ax.set_ylabel("Gamma")
# # ax.set_zlabel("Accuracy")
...
#         # Entrenar modelo SVM con datos originales
...
#         array_de_metricas = array_de_metricas + [
#             visualizarAcyF1(y_test_clasificado, y_pred_svm, 'Metrica de Modelo Kernel:', 'C:', kernel, C)
#         ]

#         # === Gráfico de frontera de decisión en 2D (PCA) ===
#         modelo = SVC(kernel=kernel, C=C)
#         modelo.fit(X_train_2D, y_train_clasificado)
#         y_pred = modelo.predict(X_test_2D)
...
#         plt.figure(figsize=(6, 5))
#         plt.contourf(xx, yy, Z, cmap=plt.cm.Greens, alpha=0.4)
#         plt.scatter(X_test_2D[:, 0], X_test_2D[:, 1], c=y_test_clasificado,
#                     cmap=plt.cm.bwr, edgecolors='k', label='Datos de prueba')
...
# # Ejes: 3 características originales
# x = X_train[:, 0]  # Horas de estudio
# y = X_train[:, 1]  # Calificaciones previas
# z = X_train[:, 2]  # Asistencia
# labels = y_train_clasificado  # 0 o 1 según aprobado/desaprobado

# # Graficar puntos 3D con colores por clase
# scatter = ax.scatter(x, y, z, c=labels, cmap='bwr', s=60, edgecolors='k')

# ax.set_xlabel("Horas de estudio")
# ax.set_ylabel("Nota previa")
# ax.set_zlabel("Asistencia")
# ax.set_title("Visualización 3D de datos originales (SVM - Aprobado/No aprobado)")
# plt.tight_layout()
# plt.show()

# # Supongamos que ya entrenaste un modelo SVM:
# modelo = SVC(kernel='rbf', C=1, gamma=1)
# modelo.fit(X_train, y_train_clasificado)

# # Vectores de soporte (índices y puntos)
# vectores_soporte = modelo.support_
...
# y = X_train[:, 1]
# z = X_train[:, 2]
# labels = y_train_clasificado

# # Colores por clase
# ax.scatter(x, y, z, c=labels, cmap='bwr', edgecolors='k', s=60)

# # Vectores de soporte resaltados
# ax.scatter(X_train[vectores_soporte, 0],
#            X_train[vectores_soporte, 1],
#            X_train[vectores_soporte, 2],
#            facecolors='none', edgecolors='k', s=100, label='Vectores de soporte')

# ax.set_xlabel("Horas de estudio")
# ax.set_ylabel("Nota previa")
# ax.set_zlabel("Asistencia")
# ax.set_title("Datos 3D + Vectores de soporte (SVM)")
# ax.legend()
# plt.tight_layout()
# plt.show()
